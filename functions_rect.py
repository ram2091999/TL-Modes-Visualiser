...
class TE_TM_Functions():
    ...
    def Fc(self):
        val = ((1/(2 * np.sqrt(mu_0 * epsilon_0))) * np.sqrt(np.power((self.m/self.a), 2) + np.power((self.n/self.b), 2)))
        return val

    def lambda_C(self):
        val = (1/np.sqrt(mu_0 * epsilon_0)) / self.Fc()
        return val

    def beta_g(self):
        fc_val = self.Fc() 
        val = self.w * np.sqrt(mu_0 * epsilon_0) * np.sqrt(1 - np.power((fc_val/self.f), 2))   
        return val
